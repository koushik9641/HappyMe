...
public class PlayerPrefsX
{
    ...
    public static bool[] GetBoolArray(String key)
    {
        if (PlayerPrefs.HasKey(key))
        {
            ...
            if (bytes.Length < 5)
            {
                DConsole.LogError("Corrupt preference file for " + key);
                return new bool[0];
            }
            if ((ArrayType)bytes[0] != ArrayType.Bool)
            {
                DConsole.LogError(key + " is not a boolean array");
                return new bool[0];
            }
            Initialize();
    ...
    public static bool SetStringArray(String key, String[] stringArray)
    {
        ...
        for (var i = 0; i < stringArray.Length; i++)
        {
            ...
            {
                DConsole.LogError("Can't save null entries in the string array when setting " + key);
                return false;
            }
            if (stringArray[i].Length > 255)
            {
                DConsole.LogError("Strings cannot be longer than 255 characters when setting " + key);
                return false;
            }
    ...
    public static String[] GetStringArray(String key)
    {
        if (PlayerPrefs.HasKey(key))
        {
            ...
            {
                DConsole.LogError(key + " is not a string array");
                return new String[0];
            }
            Initialize();
    ...
    private static void GetValue<T>(String key, T list, ArrayType arrayType, int vectorNumber, Action<T, byte[]> convert) where T : IList
    {
        if (PlayerPrefs.HasKey(key))
        {
            ...
            {
                DConsole.LogError(key + " is not a " + arrayType.ToString() + " array");
                return;
            }
            Initialize();
    ...
    public static void ShowArrayType(String key)
    {
        ...
        if (bytes.Length > 0)
        {
            ...
            DConsole.Log(key + " is a " + arrayType.ToString() + " array");
        }
    }
